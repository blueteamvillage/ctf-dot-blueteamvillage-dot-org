...
export const fallbackSetupSections: SetupSection[] = [
  {
    ...
    body: [
      ...
      "```",
      "",
      "**macOS** — [Homebrew](https://brew.sh) is the only prerequisite; make installs the rest (minikube, kubectl, helm, helmfile, k9s, colima, docker CLI):",
      "",
      "```",
      "make tools && make up",
      "```",
      "",
      "**Windows 10/11** — winget + Docker Desktop (WSL2 engine); full walkthrough and troubleshooting in [windows/README.md](https://github.com/blueteamvillage/btv-k8s-sandbox-infrastructure/blob/main/windows/README.md):",
      "",
      "```",
      "windows\\start.cmd tools    # once; start Docker Desktop, open a new terminal",
      ...
      "```",
      "",
      "Budget ~4 CPU cores, 8 GB RAM, and ~20 GB of disk, and bring a GitHub account — you'll need it to pull challenge images at the event. Do this **before** you get to the venue: the first run takes several minutes, and DEF CON internet is famously unreliable. `make stop` (Windows: `windows\\start.cmd stop`) pauses the cluster with state preserved. Stuck? Ask in the BTV Discord.",
    ].join("\n"),
  },
  {
    ...
    order: 2,
    body: [
      "Challenges are distributed as container images from the Blue Team Village registry on GitHub — one image per challenge, `ghcr.io/blueteamvillage/challenge-<NNN>`. Standalone challenges also ship **`-beginner`** and **`-pro`** variants; pick the track that fits you.",
      "",
      "**The images are private until the CTF opens at DEF CON 34.** You can't pull them ahead of time — that's by design. At the village you'll receive credentials, then pull the image and load it straight into your `dc34` cluster (the pods use `imagePullPolicy: IfNotPresent`, so the loaded image is used with no in-cluster registry setup):",
      "",
      "```",
