...
export const fallbackSetupSections: SetupSection[] = [
  ...
  {
    title: "Watch the cluster with k9s",
    tier: "everyone",
    order: 3,
    body: [
      "The single biggest quality-of-life upgrade for the con: **use [k9s](https://k9scli.io)**. It's a terminal UI over the whole cluster — instead of chaining `kubectl` commands, you get one live screen showing every pod, its status, restarts, and resource usage, with logs and a shell one keystroke away. The macOS and Windows paths install it for you (`make tools` / `windows\\start.cmd tools`); on Linux grab it from your package manager or the [k9s releases page](https://github.com/derailed/k9s/releases).",
      "",
      "Point it at the sandbox cluster and you're in:",
      "",
      "```",
      "k9s --context dc34",
      "```",
      "",
      "k9s opens on the pods view in the `default` namespace — which is empty in a fresh sandbox, so don't panic at the blank screen. Press **`0`** to show pods in **all namespaces**, and everything the sandbox runs appears at once:",
      "",
      "![k9s pods view of the dc34 cluster with all namespaces shown — Cilium, Tetragon, CoreDNS, and the control plane in kube-system, plus the four Kyverno controllers](/setup/k9s-pods-all-namespaces.png)",
      "",
      "That screen doubles as your **health check**: a freshly built sandbox shows Cilium, Tetragon, CoreDNS, metrics-server, and the control-plane pods in `kube-system`, plus four Kyverno controllers — all `Running`. Once you apply challenge manifests, their pods show up here too, in their own namespaces.",
      "",
      "The keys you'll actually use:",
      "",
      "- **`0`** — show all namespaces (**`1`** jumps back to `default`)",
      "- **`:pods`** + Enter — jump to the pods view (`:namespaces`, `:services`, `:events` work the same way)",
      "- **arrow keys** to pick a pod, then **`d`** to describe it, **`l`** for logs, **`s`** for a shell inside it",
      "- **Esc** backs out of any view, **`?`** lists every shortcut, **`:q`** quits",
      "",
      "For example, `d` on a pod gives you the full describe output — image digests, labels, IPs, container state — without ever leaving the screen:",
      "",
      "![k9s describe view of the cilium-envoy pod, showing namespace, labels, node, container image, and ports](/setup/k9s-pod-describe.png)",
      "",
      "Everything in the next section works with plain `kubectl` too — but when you're mid-investigation with malware telemetry streaming, one screen that shows the whole cluster beats twenty terminal tabs.",
    ].join("\n"),
  },
  {
    ...
    body: [
      ...
      "```",
      "",
      "(If you have k9s open from the previous step, you'll see the new namespace and pod appear the moment you apply.)",
      "",
      ...
      "",
      "Tetragon streams kernel-level events — process execs, network connections — from every workload (`kubectl --context dc34 logs -n kube-system ds/tetragon -c export-stdout -f`). That's telemetry you can hunt in. Submit your findings as flags on MetaCTF.",
    ].join("\n"),
  },
  {
